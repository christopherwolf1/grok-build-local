...
#[test]
fn inference_url_defaults_to_local_runtime() {
    let ep = endpoints("https://proxy.grok.com/v1", None, None);
    assert_eq!(
        ep.resolve_inference_base_url(),
        crate::agent::config::LOCAL_INFERENCE_BASE_URL_DEFAULT
    );
}
#[test]
...
#[test]
fn list_url_defaults_to_local_runtime_models() {
    let ep = endpoints("https://proxy.grok.com/v1", None, None);
    assert_eq!(
...
/// Local-first: list from the inference URL. Session auth only for first-party
/// xAI bearer hosts; everything else is optional API key (including loopback).
#[test]
#[serial_test::serial]
fn models_fetch_endpoint_matches_auth_mode() {
    ...
    let local = ListModelsEndpoint::from_endpoints(&default, ModelFetchAuth::ApiKey);
    assert_eq!(
        local.url,
        format!(
            "{}/models",
    ...
    assert_eq!(local.auth, EndpointAuth::OptionalApiKey);
